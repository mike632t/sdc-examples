 ...
 * command line when using SDCC to cross compiling Z80 code on CPM.
 * 
 * sdcc -mz80 --no-std-crt0 --data-loc 0 sdc-crt0-args.rel sdc-cpm.rel sdc-args.c 
 * 
 * sdobjcopy -Iihex -Obinary --gap-fill 0 sdc-args.ihx sdc-args.com
 *
 * Sort hex file
 * 
 * cat <((head sdc-args.ihx -n -1 | sort -k 1.4,1.7)) <(tail sdc-args.ihx -n -1)
 *
